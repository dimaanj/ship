...
Within the `/packages` folder, you'll find two essential packages: `create-ship-app` and `node-mongo`.
...
Learn more in the [documentation](/packages/create-ship-app).
...
It's used in the Ship template, and you can find details in the [documentation](/packages/node-mongo).
...
- **apps** - contains [API](/api-reference/overview) and [WEB](/web/overview).
- **bin** - contains scripts for setting up and starting the project.
- **packages** - contains packages with shared code (`app-constants`, `shared`, `mailer`, config packages). Find more details about package sharing in our [documentation](/package-sharing/overview).
