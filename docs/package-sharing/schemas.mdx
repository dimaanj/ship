...
Data schemas are defined inside each API resource folder (e.g. `apps/api/src/resources/users/users.schema.ts`). They are **auto-copied** to `packages/shared/src/schemas/` during codegen (`npm run generate -w shared`).
